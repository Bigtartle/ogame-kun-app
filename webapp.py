        ...
        with st.sidebar:
            ...
            def get_index(key, default_index=0, options=col_options):
                ...
                if value_to_find not in options:
                    return 0
                return options.index(value_to_find)if analysis_method == "位相直交法":
            ...
            # 自動割り当て時に、割り当てられなかった余計な列が自動でマルチセレクトに入る仕組み
            if mode_option != "手動設定":
