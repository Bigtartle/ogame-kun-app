...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.df is not None:
            ...
            mappings['B'] = st.selectbox("磁場 (B) の列", ["なし"] + col_options, index=get_index('B', 1, ["なし"] + col_options))
            mappings['Sin'] = st.selectbox("Sin(V) の列", col_options, index=get_index('Sin', 2))
            mappings['Cos'] = st.selectbox("Cos(V) の列", col_options, index=get_index('Cos', 3))
    ...
    if 'df' in st.session_state and st.session_state.df is not None:
        ...
        if 'att_run_button' in locals() and att_run_button:
            try:
                ...
                if sin_col is not None and cos_col is not None and sin_col != 'なし' and cos_col != 'なし':
